...
BASE_REW_SHAPING_PARAMS = {
    "INDGREDIENT_PICKUP_REWARD": 3,
    "INDGREDIENT_DROP_REWARD": 0,
    "PLACEMENT_IN_POT_REW": 3,
    "DISH_PICKUP_REWARD": 0,
    "SOUP_PICKUP_REWARD": 0,
    "SOUP_DELIVERY_REWARD": 20,
    "SOUP_COOKING_REWARD": 20,
...
class OvercookedGridworld(object):
    ...
    def get_state_transition(self, state, joint_action, agent_roles, display_phi=False, motion_planner=None):
        ...
        sparse_reward_by_agent, shaped_reward_by_agent, role_reward_by_agent = self.resolve_interacts(new_state, joint_action, events_infos, agent_roles)
        ...
        # if agent_roles["p0"] == 1:
        #     role_reward_by_agent[0] += cook_reward
        # if agent_roles["p1"] == 1:
        #     role_reward_by_agent[1] += cook_reward
        ...
        infos = {
            ...
            "role_reward_by_agent": role_reward_by_agent,
        }
        if display_phi:
    ...
    def resolve_interacts(self, new_state, joint_action, events_infos, agent_roles):
        """
        ...
        sparse_reward, shaped_reward, role_reward = [0] * self.num_players, [0] * self.num_players, [0] * self.num_players

        if agent_roles is None:
            player_roles = [0, 0]
        else:
            player_roles = [agent_roles["p0"], agent_roles["p1"]]
        ...
        for player_idx, (player, action) in enumerate(zip(new_state.players, joint_action)):
            ...
            if terrain_type == 'X':
                ...
                if player.has_object() and not new_state.has_object(i_pos):
                    ...
                    # if self.fc:
                    #     shaped_reward[player_idx] += self.reward_shaping_params["INDGREDIENT_DROP_REWARD"]
                    ...
                    # if self.fc:
                    #     shaped_reward[player_idx] += self.reward_shaping_params["INDGREDIENT_PICKUP_REWARD"]
                ...
                #     if player_roles[player_idx] == 2:
                #         shaped_reward[player_idx] -= self.reward_shaping_params["INDGREDIENT_PICKUP_REWARD"]
                #         # shaped_reward[player_idx] += 0
                #     elif player_roles[player_idx] == 1:
                #     # else:
                #         shaped_reward[player_idx] += self.reward_shaping_params["INDGREDIENT_PICKUP_REWARD"]
            ...
            #             soup.begin_cooking()
            #             if player_roles[player_idx] == 1:
            #                 role_reward[player_idx] += self.reward_shaping_params["SOUP_COOKING_REWARD"]
                ...
                if player.get_object().name == 'dish' and self.soup_ready_at_location(new_state, i_pos):
                    ...
                    if not soup.is_full:
                        ...
                        # if player_roles[1 - player_idx] == 1:
                        #     role_reward[1 - player_idx] -= 6
                            # shaped_reward[player_idx] += 0
                        # else:
                ...
                if obj.name == 'soup':
                    ...
                    if player_roles[1 - player_idx] == 1:
                        role_reward[1 - player_idx] -= 20
                    # elif player_roles[player_idx] == 1:
                    #     role_reward[player_idx] -= self.reward_shaping_params["SOUP_DELIVERY_REWARD"]
                        # shaped_reward[player_idx] += 0

                    # Log soup delivery
                    events_infos['soup_delivery'][player_idx] = True

        return sparse_reward, shaped_reward, role_reward
    ...
    def step_environment_effects(self, state):
        ...
        for obj in state.objects.values():
            ...
            if obj.name == 'soup':# and obj.is_cooking:
                ...
                if obj.is_cooking:
                    obj.cook()
                elif self.soup_to_be_cooked_at_location(state, s_pos):
                    # soup = new_state.get_object(i_pos)
                    if len(obj.ingredients) == 3:
                        obj.begin_cooking()
                        # reward += self.reward_shaping_params["SOUP_COOKING_REWARD"]
                        # if player_roles[player_idx] == 1:
                        #     role_reward[player_idx] += self.reward_shaping_params["SOUP_COOKING_REWARD"]
        # return reward
